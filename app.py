...
TWILIO_PHONE_NUMBER = os.environ.get("TWILIO_PHONE_NUMBER")       # For SMS (e.g., +1234567890)
TWILIO_WHATSAPP_NUMBER = os.environ.get("TWILIO_WHATSAPP_NUMBER") # For WhatsApp (e.g., +1234567890)
...
def verify_file(file_path):
    with open(file_path, "rb") as f: data = f.read()
    if SIGNATURE_MARKER not in data: return False, "No embedded signatures found"
    try:
        original_data, metadata_bytes = data.split(SIGNATURE_MARKER)
        metadata = json.loads(metadata_bytes.decode())
    except: return False, "Corrupted metadata"
    ...
    for sig in signatures:
        ...
        except: results.append(f"{signer}: INVALID SIGNATURE")
    return True, "\n".join(results)
...
def send_notification(phone_number, message):
    ...
    if not TWILIO_ACCOUNT_SID or not TWILIO_AUTH_TOKEN:
        app.logger.warning("Twilio credentials not configured.")
        return False
    ...
    try:
        ...
        if TWILIO_PHONE_NUMBER:
            ...
            app.logger.info(f"✅ SMS sent to {phone_number}")
            
        # Send WhatsApp
        if TWILIO_WHATSAPP_NUMBER:
            client.messages.create(body=message, from_=f"whatsapp:{TWILIO_WHATSAPP_NUMBER}", to=f"whatsapp:{phone_number}")
            app.logger.info(f"✅ WhatsApp sent to {phone_number}")
            
        return True
    except Exception as e:
        app.logger.error(f"❌ Notification failed for {phone_number}: {e}")
        return False
...
def forward_document(doc_id):
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT stored_file, step, document_name FROM documents WHERE id=%s", (doc_id,))
        row = cur.fetchone()
        if not row:
            flash("Document not found")
            return redirect("/incoming")

        stored_file, current_step, doc_name = row
        new_step = current_step + 1
        file_path = os.path.join(SIGNED_FOLDER, stored_file)
        if not os.path.exists(file_path): raise Exception("Source file not found")

        private_key = load_private_key(session["username"], session["password"])
        new_file = sign_file(file_path, session["username"], private_key, comment)

        cur.execute("UPDATE documents SET stored_file=%s, current_holder=%s, status=%s, step=%s WHERE id=%s", (new_file, forward_to, f"FORWARDED TO {forward_to}", new_step, doc_id))
        cur.execute("INSERT INTO document_history (document_id, signer, comment, action, step) VALUES (%s, %s, %s, %s, %s)", (doc_id, session["username"], comment, f"FORWARDED TO {forward_to}", new_step))
        if copy_to and copy_to != "":
            cur.execute("INSERT INTO document_history (document_id, signer, comment, action, step) VALUES (%s, %s, %s, %s, %s)", (doc_id, session["username"], comment, f"CC TO {copy_to}", new_step))
        ...
        cur.execute("SELECT phone_number FROM users WHERE username = %s", (forward_to,))
        rec_row = cur.fetchone()
        if rec_row and rec_row[0]:
            msg = f"Hi {forward_to}, {session['username']} forwarded a document to you: '{doc_name}'. Comment: {comment}"
            send_notification(rec_row[0], msg)

        # Notify the CC person
        if copy_to and copy_to != "":
            cur.execute("SELECT phone_number FROM users WHERE username = %s", (copy_to,))
            cc_row = cur.fetchone()
            if cc_row and cc_row[0]:
                msg = f"Hi {copy_to}, you were CC'd on a document from {session['username']}: '{doc_name}'. Comment: {comment}"
                send_notification(cc_row[0], msg)
        # =========================================
...
# =========================================
# 👤 PROFILE ROUTE (ADD/CHANGE PHONE)
# =========================================
...
def profile():
    ...
    if request.method == "POST":
        ...
        cur.execute("UPDATE users SET phone_number = %s WHERE username = %s", (phone, session["username"]))
        conn.commit()
        flash("Phone number updated successfully!")
    
    cur.execute("SELECT phone_number FROM users WHERE username = %s", (session["username"],))
    row = cur.fetchone()
    conn.close()
    
    current_phone = row[0] if row and row[0] else ""
    return render_template("profile.html", phone_number=current_phone)
