...
def migrate_db():
    ...
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='documents' AND column_name='step') THEN
            ALTER TABLE documents ADD COLUMN step INTEGER DEFAULT 1;
        END IF;
    END $$;
    """)
    ...
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='document_history' AND column_name='step') THEN
            ALTER TABLE document_history ADD COLUMN step INTEGER DEFAULT 1;
        END IF;
    END $$;
    """)
...
def dashboard():
    ...
    if request.method == "POST":
        try:
            ...
            VALUES (%s, %s, %s, %s, %s)
            """, (document_id, session["username"], comment, "DOCUMENT CREATED", 1))
            conn.commit()
            conn.close()
...
def document_details(doc_id):
    ...
    FROM document_history WHERE document_id=%s ORDER BY id ASC
    """, (doc_id,))
...
def forward(doc_id):
    ...
    if request.method == "POST":
        try:
            ...
            cur.execute("""
            UPDATE documents SET stored_file=%s, current_holder=%s, status=%s, step=%s WHERE id=%s
            """, (new_file, next_user, f"FORWARDED TO {next_user}", new_step, doc_id))

            cur.execute("""
            INSERT INTO document_history (document_id, signer, comment, action, step)
            VALUES (%s, %s, %s, %s, %s)
            """, (doc_id, session["username"], comment, f"FORWARDED TO {next_user}", new_step))
...
def incoming():
    ...
    cur = conn.cursor()
    cur.execute("""
    SELECT id, document_name, created_by, status FROM documents
    WHERE current_holder=%s ORDER BY id DESC
    """, (session["username"],))
    docs = cur.fetchall()
    conn.close()
...
def sent():
    ...
    cur = conn.cursor()
    cur.execute("""
    SELECT id, document_name, current_holder, status, step, created_at FROM documents
    WHERE created_by=%s ORDER BY id DESC
    """, (session["username"],))
    docs = cur.fetchall()
    conn.close()
...
def approve_document(doc_id):
    ...
    INSERT INTO document_history (document_id, signer, comment, action, step)
    VALUES (%s, %s, %s, %s, %s)
    """, (doc_id, session["username"], "Document Approved", "APPROVED", current_step))
    conn.commit()
    conn.close()
