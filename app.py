...
def migrate_db():
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            WHERE table_name='documents' AND column_name='step'
        ) THEN
            ALTER TABLE documents ADD COLUMN step INTEGER DEFAULT 0;
        END IF;
    END $$;
    """)
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name='document_history' AND column_name='step'
        ) THEN
            ALTER TABLE document_history ADD COLUMN step INTEGER DEFAULT 0;
        END IF;
    END $$;
    """)
